...
if __name__=='__main__':
    ...
    if len(sys.argv) == 6:
        ...
        J_filter    = sys.argv[2]
        H_filter    = sys.argv[3]
        Hw_low      = float(sys.argv[4])
        Hw_up       = float(sys.argv[5])

        result      = main(regex_spec, J_filter, H_filter, Hw_low, Hw_up)

        print(result)
    else:
        print("usage) [spectra dir] [J band filter] [H band filter] [Hw lower] [Hw upper]")
        print("ex) "+sys.argv[0]+" ./spectra/ ./filter/J_filter.dat ./filter/H_filter.dat 9000 15000")
