...
from telescope_baseline.utils.parameters import Parameters


class InstClass(object):
    """ Instrument Class

    Class for Instrumental Setting used in exocounts,
    equivalent to InstClass in exocounts but using Parameters.
    """

    def __init__(self):

        par = Parameters.get_instance()
        self.lamb = (par.long_wavelength_limit +
                     par.short_wavelength_limit) / 2.0 * u.m
        self.dlam = (par.long_wavelength_limit -
                     par.short_wavelength_limit) * u.m
...
class ObsClass(object):
    ...
    def calc_noise(self):
        ...
        try:
            self.sigr = np.sqrt(self.mu * self.napix * self.texposure.to(u.s) /
                                self.tframe) * self.inst.nread
        ...
        # foreground
        try:
            self.nphoton_foreground = (self.fgaparture *
                                       (self.inst.fgtel + self.inst.fgatm) *
                                       self.inst.dlam * self.texposure *
                                       (self.inst.dtel / 2.0)**2 * np.pi *
                                       self.inst.throughput).to(1)
        except:
